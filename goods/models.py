...
# здесь будет ссылка на изображение,а само изображение будет храниться в папке media
    image = models.ImageField(upload_to='goods_images',
                              blank=True, null=True, verbose_name='Изображения')
    ...
    # protect не дает удалить категорию если есть товары,есть сет денфолт,а каскад уд все товары в категории,но дает предупреждение
